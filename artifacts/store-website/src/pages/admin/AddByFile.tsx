...
import {
  Upload, Loader2, AlertCircle, CheckCircle2, FileArchive,
  ArrowRight, Star, Bell, RefreshCw, ChevronLeft, X
} from "lucide-react";
...
export default function AdminAddByFile() {
  ...
  const [step, setStep] = useState<"file" | "parsing" | "form">("file");
  ...
  const handleFile = async (file: File) => {
    if (!file.name.toLowerCase().endsWith(".ipa")) {
      setParseError("الملف يجب أن يكون بصيغة .ipa");
      return;
    }
    ...
    <AdminLayout>
      <div className="max-w-2xl mx-auto" dir="rtl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-white text-sm">إضافة عن طريق ملف</span>
        </div>
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">إضافة تطبيق عبر رفع ملف</h1>
            <p className="text-sm text-white/40">ارفع ملف IPA وسيتم استخراج البيانات تلقائياً</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 space-y-5">
              ...
              <div
                ...
                style={{
                  borderColor: dragOver ? ACCENT : "rgba(255,255,255,0.08)",
                  background: dragOver ? `${ACCENT}05` : "transparent",
                }}
              >
                <div className="w-14 h-14 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ background: `${ACCENT}12` }}>
                  <FileArchive className="w-7 h-7" style={{ color: ACCENT }} />
                </div>
                <p className="text-white font-bold text-base">اسحب ملف IPA هنا</p>
                <p className="text-white/30 text-sm mt-1">أو اضغط لاختيار الملف</p>
                <p className="text-white/20 text-xs mt-3">الصيغة المدعومة: .ipa — الحد الأقصى: 500 MB</p>
              </div>
              ...
              <div className="bg-white/3 rounded-xl p-4 space-y-2">
                <p className="text-xs font-medium text-white/60">ملاحظات</p>
                <ul className="space-y-1.5 text-xs text-white/30">
                  <li className="flex items-start gap-2"><span style={{ color: ACCENT }}>•</span> الملفات الكبيرة (أكثر من 500MB) استخدم "إضافة عبر رابط"</li>
                  <li className="flex items-start gap-2"><span style={{ color: ACCENT }}>•</span> يتم استخراج الاسم والبندل والأيقونة تلقائياً</li>
                  <li className="flex items-start gap-2"><span style={{ color: ACCENT }}>•</span> الملف يُرفع وتُستخرج منه المعلومات فقط</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-16 flex flex-col items-center gap-5 text-center">
            <div className="relative">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: `${ACCENT}15` }}>
                <FileArchive className="w-8 h-8" style={{ color: ACCENT }} />
              </div>
            ...
            <div>
              ...
              {selectedFile && <p className="text-white/40 text-sm mt-1">{selectedFile.name}</p>}
            ...
            <div className="w-full max-w-xs bg-white/5 rounded-full h-1.5 overflow-hidden">
              <div className="h-full rounded-full animate-pulse" style={{ background: ACCENT, width: "70%" }} />
            </div>
          </div>
        )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {parsed && (
              <div className="bg-[#0a0a0a] border rounded-2xl p-4 flex items-center gap-4" style={{ borderColor: `${ACCENT}25` }}>
                ...
                <button type="button" onClick={() => setStep("file")} className="flex items-center gap-1.5 text-xs text-white/30 hover:text-white transition-colors shrink-0">
            ...
            <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  <FieldGroup label="رابط التحميل (IPA) - اختياري">
